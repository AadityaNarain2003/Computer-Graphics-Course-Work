...
Ray Camera::generateRay_Anti_aliasing(int x, int y)
{
    float x_m=next_float();
    float y_m=next_float();
    //printf("%f %f\n",x_m,y_m);
    Vector3f pixelCenter = this->upperLeft + (x_m*this->pixelDeltaU) + (y_m*this->pixelDeltaV);
    pixelCenter = pixelCenter + x * this->pixelDeltaU + y * this->pixelDeltaV;
