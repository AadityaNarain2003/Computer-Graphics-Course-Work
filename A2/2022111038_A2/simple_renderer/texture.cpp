...
Vector3f Texture::nearestNeighbourFetch(const Vector2f &uv)
{
    // Map UV coordinates to texture space
    float u = uv.x * (this->resolution[0]-2 ); // Assuming width is the width of the texture image
    float v = uv.y * (this->resolution[1]-2 ); // Assuming height is the height of the texture image

    // Nearest neighbor search
    int x = std::round(u);
    int y = std::round(v);

    // Ensure the sampled coordinates are within the texture bounds
    x = std::max(0, std::min(this->resolution[0] - 2, x));
    y = std::max(0, std::min(this->resolution[1] - 2, y));

    // Sample the color at the nearest neighbor pixel
    return this->loadPixelColor(x, y);
}

Vector3f Texture::bilinearFetch(const Vector2f &uv)
{
    // Map UV coordinates to texture space
    float u = uv.x * (this->resolution[0]-2 );
    float v = uv.y * (this->resolution[1]-2 );

    // Get integer coordinates of the bottom-left pixel
    int x0 = std::floor(u);
    int y0 = std::floor(v);

    // Get fractional parts of the UV coordinates
    float dx = u - x0;
    float dy = v - y0;

    // Ensure sampled coordinates are within texture bounds
    x0 = std::max(0, std::min(this->resolution[0] - 2, x0));
    y0 = std::max(0, std::min(this->resolution[1] - 2, y0));

    // Sample colors from the four surrounding pixels
    Vector3f c00 = this->loadPixelColor(x0, y0);
    Vector3f c10 = this->loadPixelColor(x0 + 1, y0);
    Vector3f c01 = this->loadPixelColor(x0, y0 + 1);
    Vector3f c11 = this->loadPixelColor(x0 + 1, y0 + 1);

    // Perform bilinear interpolation
    Vector3f color =
        (1 - dx) * (1 - dy) * c00 + dx * (1 - dy) * c10 +
