...
void Surface::intersectBVH(uint32_t nodeIdx, Ray& ray, Interaction& si)
{
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            siIntermediate.uv3=this->tris[this->getIdx(i + node.firstPrim)].uv3;

            siIntermediate.v1=this->tris[this->getIdx(i + node.firstPrim)].v1;
            siIntermediate.v2=this->tris[this->getIdx(i + node.firstPrim)].v2;
            siIntermediate.v3=this->tris[this->getIdx(i + node.firstPrim)].v3;
