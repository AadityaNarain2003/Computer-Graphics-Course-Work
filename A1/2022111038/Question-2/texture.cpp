...
Texture::Texture(std::string pathToImage)
{
    ...
    if (pos > pathToImage.length()) {
        ...
        this->type = TextureType::double_ALPHA;
...
void Texture::allocate(TextureType type, Vector2i resolution)
{
    ...
    else if (this->type == TextureType::double_ALPHA) {
        double* dpointer = (double*)malloc(this->resolution.x * this->resolution.y * 4 * sizeof(double));
        this->data = (uint64_t)dpointer;
    }
}
...
{
    if (this->type == TextureType::double_ALPHA) {
        ...
        std::cerr << "Cannot save to EXR: texture is not of type double." << std::endl;
    }
}
